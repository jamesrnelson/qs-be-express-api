  ...
  describe("GET /api/v1/favorite_foods", () => {
    it("returns times eaten with the foods that have been eaten that many times", (done) => {
      ...
      .end((err, res) => {
        ...
        expect(res.body[0].timesEaten).to.eql('1');
        expect(res.body[0].foods.length).to.eql(5);
        expect(res.body[0].foods[0].name).to.eql("Yogurt");
        expect(res.body[0].foods[0].calories).to.eql(550);
