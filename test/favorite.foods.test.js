  ...
  describe("GET /api/v1/favorite_foods", () => {
    it("returns times eaten with the foods that have been eaten that many times", (done) => {
      ...
      .end((err, res) => {
        ...
        expect(res.body[1].foods.length).to.eql(1);
        expect(res.body[1].foods[0].name).to.eql("Apple");
        expect(res.body[1].foods[0].calories).to.eql(220);
        expect(res.body[2].timesEaten).to.eql(1);
        expect(res.body[2].foods.length).to.eql(3);
        done();
      });
    });
  });
